...
Cubre:
  LP-148 — Modificar datos de contacto del envío  (CP-0209, CP-0210, CP-0211, CP-0212, CP-0213)
  LP-154 — Modificar datos operativos del envío   (CP-0216, CP-0217, CP-0218, CP-0219, CP-0220,
                                                   CP-0221, CP-0222, CP-0223)

Tests NO implementados (requieren autenticación JWT, no disponible en este prototipo):
  CP-0206, CP-0207, CP-0208 — requieren JWT con roles
  CP-0214, CP-0215          — requieren JWT con roles
  CP-0224                   — requiere JWT con roles
"""
...
class TestCP0209PersistenciaContacto:
    ...
    def test_cp0209_patch_contacto_retorna_200(self, client):
        """CP-0209 (HP) — PATCH /envios/{tid}/contacto con datos válidos retorna 200."""
        tid = _crear_envio(client)
        resp = client.patch(f"/envios/{tid}/contacto", json=PAYLOAD_CONTACTO_VALIDO)
        assert resp.status_code == 200

    def test_cp0209_destinatario_actualizado_en_bd(self, client, db_session):
        """CP-0209 (HP) — El destinatario queda persistido en la BD tras el PATCH."""
        tid = _crear_envio(client)
        client.patch(f"/envios/{tid}/contacto", json=PAYLOAD_CONTACTO_VALIDO)
        envio = db_session.query(Envio).filter(Envio.tracking_id == tid).first()
        assert envio.destinatario == "Carlos López"

    def test_cp0209_direccion_destino_actualizada_en_bd(self, client, db_session):
...
class TestCP0210ValidacionContacto:
    ...
    def test_cp0210_calle_solo_numerica_retorna_422(self, client):
        ...
        assert client.patch(f"/envios/{tid}/contacto", json=payload).status_code == 422

    def test_cp0210_numero_no_numerico_retorna_422(self, client):
    ...
    def test_cp0210_sin_persistencia_si_validacion_falla(self, client, db_session):
        """CP-0210 (UP) — Si el PATCH falla por validación, los datos originales no cambian."""
        tid = _crear_envio(client)
        payload = {**PAYLOAD_CONTACTO_VALIDO,
                   "direccion_destino": {**PAYLOAD_CONTACTO_VALIDO["direccion_destino"], "numero": "ABC"}}
        client.patch(f"/envios/{tid}/contacto", json=payload)
        envio = db_session.query(Envio).filter(Envio.tracking_id == tid).first()
...
class TestCP0211TrackingIdNoEditable:
    ...
    def test_cp0211_tracking_id_no_cambia_si_se_incluye_en_body(self, client):
...
class TestCP0212ConfirmacionContacto:
    ...
    def test_cp0212_respuesta_incluye_nuevo_destinatario(self, client):
    ...
    def test_cp0212_respuesta_incluye_nueva_direccion_destino(self, client):
...
class TestCP0213CamposObligatoriosContacto:
    ...
    def test_cp0213_destinatario_vacio_retorna_422(self, client):
        """CP-0213 (UP) — PATCH con destinatario vacío retorna 422."""
        tid = _crear_envio(client)
        payload = {**PAYLOAD_CONTACTO_VALIDO, "destinatario": ""}
        assert client.patch(f"/envios/{tid}/contacto", json=payload).status_code == 422

    def test_cp0213_ciudad_con_numeros_retorna_422(self, client):
    ...
    def test_cp0213_codigo_postal_no_numerico_retorna_422(self, client):
...
class TestCP0216EdicionFechaEntrega:
    ...
    def test_cp0216_patch_operativo_con_fecha_valida_retorna_200(self, client):
        """CP-0216 (HP) — PATCH /envios/{tid}/operativo con fecha futura válida retorna 200."""
        tid = _crear_envio(client)
        resp = client.patch(f"/envios/{tid}/operativo", json=PAYLOAD_OPERATIVO_VALIDO)
        assert resp.status_code == 200

    def test_cp0216_fecha_persistida_en_bd(self, client, db_session):
        """CP-0216 (HP) — La nueva fecha queda persistida en la BD."""
        tid = _crear_envio(client)
        client.patch(f"/envios/{tid}/operativo", json=PAYLOAD_OPERATIVO_VALIDO)
        envio = db_session.query(Envio).filter(Envio.tracking_id == tid).first()
...
class TestCP0217FechaPasadaRechazada:
    ...
    def test_cp0217_fecha_pasada_retorna_422(self, client):
        ...
        assert client.patch(f"/envios/{tid}/operativo", json=payload).status_code == 422

    def test_cp0217_fecha_original_no_cambia_si_validacion_falla(self, client, db_session):
        """CP-0217 (UP) — Con fecha inválida, la fecha original no cambia en BD."""
        tid = _crear_envio(client)
        payload = {**PAYLOAD_OPERATIVO_VALIDO, "fecha_entrega_estimada": _FECHA_PASADA}
        client.patch(f"/envios/{tid}/operativo", json=payload)
        envio = db_session.query(Envio).filter(Envio.tracking_id == tid).first()
...
class TestCP0218EdicionProbabilidad:
    ...
    def test_cp0218_prioridad_recalculada_tras_patch_operativo(self, client):
    ...
    def test_cp0218_probabilidad_persistida_en_bd(self, client, db_session):
...
class TestCP0219ProbabilidadVacia:
    ...
    def test_cp0219_prob_ausente_retorna_422(self, client):
        """CP-0219 (UP) — PATCH sin probabilidad_retraso retorna 422."""
        tid = _crear_envio(client)
        payload = {"fecha_entrega_estimada": _FECHA_LEJANA}
        assert client.patch(f"/envios/{tid}/operativo", json=payload).status_code == 422

    def test_cp0219_prob_null_retorna_422(self, client):
...
class TestCP0220PrioridadNoEditable:
    ...
    def test_cp0220_no_existe_endpoint_directo_para_editar_prioridad(self, client):
    ...
    def test_cp0220_prioridad_en_body_es_ignorada_y_calculada_por_ml(self, client):
        ...
        resp = client.patch(f"/envios/{tid}/operativo", json=payload)
        assert resp.status_code == 200
        assert resp.json()["prioridad"] in {"ALTA", "MEDIA", "BAJA"}
...
class TestCP0221ValidacionFormatoFecha:
    ...
    def test_cp0221_fecha_texto_invalido_retorna_422(self, client):
        """CP-0221 (UP) — PATCH con texto 'hoy' como fecha retorna 422."""
        tid = _crear_envio(client)
        payload = {**PAYLOAD_OPERATIVO_VALIDO, "fecha_entrega_estimada": "hoy"}
        assert client.patch(f"/envios/{tid}/operativo", json=payload).status_code == 422

    def test_cp0221_fecha_formato_dd_mm_yyyy_retorna_422(self, client):
...
class TestCP0223ConfirmacionOperativo:
    ...
    def test_cp0223_respuesta_incluye_nueva_fecha(self, client):
    ...
    def test_cp0223_respuesta_incluye_nueva_probabilidad(self, client):
    ...
    def test_cp0223_respuesta_incluye_prioridad_recalculada(self, client):
