...
def test_cp0175_campos_obligatorios_marcados_con_asterisco(page: Page, base_url):
    """CP-0175 — Happy Path: todos los campos obligatorios muestran indicador visual (*)."""
    page.goto(base_url)
    page.click("#tab-form")
    page.wait_for_selector("#alta-form", state="visible")

    # El subtítulo del formulario debe mencionar que * indica campo obligatorio
    subtitulo = page.locator("#view-form .page-subtitle")
    expect(subtitulo).to_contain_text("*")

    # Todos los labels de campos obligatorios deben tener span.req con el asterisco
    indicadores = page.locator("#alta-form span.req")
    count = indicadores.count()
    assert count > 0, "No se encontraron indicadores de campos obligatorios (span.req)"

    # Verificar que los campos clave tienen su asterisco
...
def test_cp0260_formulario_usable_en_cualquier_dispositivo(page: Page, base_url, viewport, nombre):
    """CP-0260 — Happy Path: todos los campos del formulario son accesibles y funcionales en cualquier tamaño de pantalla."""
    ...
    for selector in campos:
        ...
        assert campo.is_visible(), f"[{nombre}] Campo {selector} no es visible"
