...
def test_cp0175_campos_obligatorios_marcados_con_asterisco
(page: Page, base_url):
    ...
    # El subtítulo debe mencionar que * indica campo obligatorio
    subtitulo = page.locator("#view-form .page-subtitle")
    expect(subtitulo).to_contain_text("*")

    # Todos los labels de campos obligatorios con span.req con el asterisco
    ...
    for campo_id in campos_obligatorios:
        ...
        assert label.count() > 0, f"El campo '{campo_id}' 
        no tiene indicador de obligatorio"
...
def test_cp0200_busqueda_vacia_no_falla(page: Page, base_url):
    ...
    assert page.locator("#empty-state, #envios-table, 
    #no-results").count() > 0, (
...
(page: Page, base_url, viewport, nombre):
    ...
    for selector in campos:
        ...
        assert campo.is_visible(), f"[{nombre}] 
        Campo {selector} no es visible"
