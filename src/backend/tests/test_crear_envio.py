...
Cubre:
  ...
  LP-21  — Autenticación requerida           (CP-0001, CP-0002)
"""
import pytest
...
# ── LP-21 — Control de acceso por autenticación ───────────────────────────────

def test_cp0001_sin_token_retorna_401(client):
    """CP-0001 — Sin header Authorization → 401 Unauthorized."""
    assert client.post("/envios/", json=PAYLOAD_VALIDO).status_code == 401


def test_cp0002_token_invalido_retorna_401(client):
    """CP-0002 — Token inválido → 401 Unauthorized."""
    headers = {"Authorization": "Bearer token.invalido.aqui"}
    assert client.post("/envios/", json=PAYLOAD_VALIDO, headers=headers).status_code == 401


# ── LP-2 / LP-131 — Campos obligatorios y validaciones ───────────────────────

def test_cp0003_crear_envio_datos_validos(client, headers_operador):
    """CP-0003 / CP-0006 / CP-0176 / CP-0179 / CP-0181 — Happy Path: POST con todos los campos válidos retorna 201."""
    response = client.post("/envios/", json=PAYLOAD_VALIDO, headers=headers_operador)
    assert response.status_code == 201


def test_cp0004_remitente_vacio_retorna_422(client, headers_operador):
    """CP-0004 / CP-0177 / CP-0180 / CP-0182 — Unhappy Path: remitente vacío → 422."""
    payload = {**PAYLOAD_VALIDO, "remitente": ""}
    assert client.post("/envios/", json=payload, headers=headers_operador).status_code == 422


def test_cp0005_remitente_solo_espacios_retorna_422(client, headers_operador):
    """CP-0005 — Edge Case: remitente con solo espacios debe ser rechazado con 422."""
    payload = {**PAYLOAD_VALIDO, "remitente": "   "}
    assert client.post("/envios/", json=payload, headers=headers_operador).status_code == 422


def test_cp0007_numero_domicilio_con_letras_retorna_422(client, headers_operador):
...
def test_cp0008_envio_creado_aparece_en_listado(client, headers_operador):
...
def test_cp0009_envio_fallido_no_aparece_en_listado(client, headers_operador):
    """CP-0009 — Unhappy Path: un envío rechazado no aparece en el listado."""
    client.post("/envios/", json={**PAYLOAD_VALIDO, "remitente": ""}, headers=headers_operador)
    resp = client.get("/envios/", headers=headers_operador)
    assert resp.json()["total"] == 0


def test_cp0174_error_422_incluye_detalle_del_campo(client, headers_operador):
    """CP-0174 — NFR: el error 422 incluye mensajes descriptivos con el campo afectado."""
    payload = {**PAYLOAD_VALIDO, "remitente": ""}
    resp = client.post("/envios/", json=payload, headers=headers_operador)
...
def test_cp0178_formato_invalido_y_campo_vacio_retorna_422(client, headers_operador):
...
def test_cp0183_campos_direccion_validos_retorna_201(client, headers_operador):
    """CP-0183 — Happy Path: campos de dirección con formato correcto → 201."""
    assert client.post("/envios/", json=PAYLOAD_VALIDO, headers=headers_operador).status_code == 201


def test_cp0184_campo_direccion_vacio_retorna_422(client, headers_operador):
...
def test_cp0185_fecha_entrega_ausente_retorna_422(client, headers_operador):
...
def test_cp0094_tracking_id_generado_automaticamente(client, headers_operador):
    """CP-0094 — Happy Path: el sistema genera tracking_id automático con formato LT-XXXXXXXX."""
    resp = client.post("/envios/", json=PAYLOAD_VALIDO, headers=headers_operador)
    assert resp.status_code == 201
    assert resp.json()["tracking_id"].startswith("LT-")


def test_cp0095_tracking_ids_son_unicos(client, headers_operador):
    """CP-0095 — Happy Path: dos envíos consecutivos reciben tracking IDs distintos."""
    r1 = client.post("/envios/", json=PAYLOAD_VALIDO, headers=headers_operador)
    r2 = client.post("/envios/", json=PAYLOAD_VALIDO, headers=headers_operador)
    assert r1.status_code == r2.status_code == 201
    assert r1.json()["tracking_id"] != r2.json()["tracking_id"]


def test_cp0096_tracking_id_visible_en_respuesta(client, headers_operador):
...
def test_cp0097_no_existe_endpoint_para_editar_tracking_id(client, headers_operador):
...
def test_cp0150_prioridad_persistida_al_crear_con_prob_retraso(client, headers_operador):
    """CP-0150 (HP) — CA-1: POST con probabilidad_retraso → prioridad asignada automáticamente en la respuesta."""
    resp = client.post("/envios/", json=_PAYLOAD_CON_PROB, headers=headers_operador)
    assert resp.status_code == 201
    assert resp.json()["prioridad"] in _PRIORIDADES_VALIDAS


def test_cp0151_probabilidad_invalida_retorna_422_sin_crear_envio(client, headers_operador):
    """CP-0151 (UP) — CA-1: probabilidad_retraso fuera de rango → 422, envío no creado."""
    payload = {**PAYLOAD_VALIDO, "probabilidad_retraso": 1.5}
    resp = client.post("/envios/", json=payload, headers=headers_operador)
    assert resp.status_code == 422
    assert client.get("/envios/", headers=headers_operador).json()["total"] == 0


def test_cp0152_prioridad_en_respuesta_pertenece_a_valores_validos(client, headers_operador):
...
def test_cp0153_prioridad_no_es_editable_manualmente(client, headers_operador):
...
def test_cp0313_payload_vacio_devuelve_422_con_detalle(client, headers_operador):
    """CP-0313 — Edge Case: POST con payload vacío retorna 422 con detalle de los campos faltantes."""
    res = client.post("/envios/", json={}, headers=headers_operador)
