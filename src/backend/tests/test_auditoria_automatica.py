...
def _crear_envio(client) -> tuple[str, str]:
...
def _avanzar_hasta_cancelado(client, tid: str):
...
class TestCP0103CreacionAutomatica:
    ...
    def test_cp0103_se_genera_un_evento_al_crear_envio(self, client, db_session):
        """CP-0103 (HP) — CA-2: Al crear un envío se genera exactamente 1 EventoDeEnvio."""
        _, uuid = _crear_envio(client)
        eventos = _eventos_de(db_session, uuid)
        assert len(eventos) == 1

    def test_cp0103_evento_tiene_accion_creacion(self, client, db_session):
    ...
    def test_cp0103_evento_estado_inicial_es_null(self, client, db_session):
        """CP-0103 (HP) — CA-2: El EventoDeEnvio de creación tiene estado_inicial = None."""
        _, uuid = _crear_envio(client)
        evento = _eventos_de(db_session, uuid)[0]
        assert evento.estado_inicial is None

    def test_cp0103_evento_estado_final_es_registrado(self, client, db_session):
    ...
    def test_cp0103_evento_tiene_usuario_fecha_y_envio(self, client, db_session):
...
class TestCP0104ModificacionAutomatica:
    ...
    def test_cp0104_patch_contacto_genera_evento_modificacion(self, client, db_session):
        """CP-0104 (HP) — CA-3: PATCH /contacto genera EventoDeEnvio con accion = MODIFICACION."""
        tid, uuid = _crear_envio(client)
    ...
    def test_cp0104_patch_operativo_genera_evento_modificacion(self, client, db_session):
    ...
    def test_cp0104_estado_inicial_y_final_son_estado_actual(self, client, db_session):
...
class TestCP0105ModificacionFallida:
    ...
    def test_cp0105_tracking_inexistente_retorna_404(self, client):
        """CP-0105 (UP) — CA-3: PATCH sobre tracking inexistente devuelve 404."""
        r = client.patch("/envios/LT-99999999/contacto", json=CONTACTO_NUEVO)
        assert r.status_code == 404

    def test_cp0105_operacion_fallida_no_genera_evento(self, client, db_session):
        ...
        client.patch("/envios/LT-99999999/contacto", json=CONTACTO_NUEVO)
...
class TestCP0106ModificacionEliminado:
    ...
    def test_cp0106_modificar_eliminado_retorna_409(self, client):
        """CP-0106 (EC) — CA-3: PATCH sobre envío ELIMINADO devuelve 409."""
        tid, _ = _crear_envio(client)
        _avanzar_hasta_cancelado(client, tid)
        client.delete(f"/envios/{tid}")
        r = client.patch(f"/envios/{tid}/contacto", json=CONTACTO_NUEVO)
        assert r.status_code == 409

    def test_cp0106_modificar_eliminado_no_genera_nuevo_evento(self, client, db_session):
...
class TestCP0107CambioEstadoAutomatico:
    ...
    def test_cp0107_cambio_estado_genera_evento(self, client, db_session):
    ...
    def test_cp0107_evento_registra_estado_inicial_y_final_correctos(self, client, db_session):
    ...
    def test_cp0107_evento_registra_ubicacion(self, client, db_session):
...
class TestCP0108EliminacionAutomatica:
    ...
    def test_cp0108_eliminacion_genera_evento(self, client, db_session):
    ...
    def test_cp0108_evento_estado_inicial_es_cancelado(self, client, db_session):
    ...
    def test_cp0108_evento_estado_final_es_eliminado(self, client, db_session):
...
class TestCP0109SinIntervencion:
    ...
    def test_cp0109_evento_creado_sin_campo_extra_en_request(self, client, db_session):
    ...
    def test_cp0109_multiples_acciones_generan_eventos_automaticamente(self, client, db_session):
...
class TestCP0111MovimientoAutomatico:
    ...
    def test_cp0111_movimiento_genera_evento(self, client, db_session):
    ...
    def test_cp0111_evento_estado_inicial_igual_final(self, client, db_session):
    ...
    def test_cp0111_evento_registra_ubicacion_nueva(self, client, db_session):
...
class TestCP0112MovimientoInvalido:
    ...
    def test_cp0112_ubicacion_faltante_retorna_422(self, client):
        """CP-0112 (UP) — CA-7: POST /movimientos sin ubicación devuelve 422."""
        tid, _ = _crear_envio(client)
        r = client.post(f"/envios/{tid}/movimientos", json={})
        assert r.status_code == 422

    def test_cp0112_ubicacion_invalida_no_genera_evento(self, client, db_session):
